...
3. Tool responses and other events

To do this, apply both [`"messages"`](#llm-tokens) and [`"updates"`](#agent-progress) streaming modes. The `"messages"` streaming mode will include [message chunks](/oss/python/langchain/messages#streaming-and-chunks) from all LLM calls in the agent. The `"updates"` mode will include completed messages that are tracked in the agent's [state](/oss/python/langchain/agents#memory).

<Note>
If outputs from LLM calls are tracked in the agent's state, you can access aggregated messages (including parsed tool calls) using `stream_mode="updates"`. Below, we demonstrate how to aggregate the completed message from any LLM call.
</Note>
<Note>
...
for stream_mode, data in agent.stream(
    ...
    if stream_mode == "messages":
        # Handle LLM streaming output
        token, metadata = data

        if isinstance(token, AIMessageChunk):
            # Can also use `metadata` to subset by node or other parameters
    ...
    if stream_mode == "updates":
        for source, update in data.items():
            # `source` is the name of the node generating the update
            if source == "tools":
                tool_message = update["messages"][-1]
                print(f"Tool response: {tool_message.content_blocks}")
```
```shell title="Output" expandable
