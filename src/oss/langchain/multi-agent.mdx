...
| [**Skills**](#skills)             | Specialized prompts are invoked on-demand. The agent loads information progressively rather than upfront.                                                       | **On-demand**: skills are loaded as needed.                    | Progressive disclosure, team distribution       |
...
At the center of multi-agent design is **[context engineering](/oss/langchain/context-engineering)** - deciding what information each agent sees. LangGraph gives you fine-grained control over:
...
* **Prompt-driven specialization**: Skills are primarily defined by specialized prompts and instructions
* **Progressive disclosure**: Skills are loaded only when needed, reducing context usage
...
* You need to **manage large amounts of domain knowledge** (e.g., database schemas, API docs)
* Different **teams need to develop capabilities independently**
* You want **progressive disclosure** — reveal capabilities as context demands

Below are some examples:

- **SQL assistant**: Skills for different business verticals (sales analytics, inventory management) with domain-specific schemas
...
1. **Lightweight descriptions**: The agent's system prompt contains brief descriptions of available skills
2. **On-demand loading**: When the agent identifies a relevant skill, it calls a tool to load the full skill content
3. **Context injection**: The skill's detailed prompts, schemas, or instructions are injected into the conversation
4. **Task execution**: The agent uses the loaded skill to complete the task

This approach differs from supervisors (where sub-agents run independently) and handoffs (where control transfers). With skills, the main agent stays in control and simply gains additional context.
