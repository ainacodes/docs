...
Below we include short, end-to-end snippets representing common use-cases for streaming.
...
To do this, apply both [`"messages"`](#llm-tokens) and [`"updates"`](#agent-progress) streaming modes. The `"messages"` streaming mode will include [message chunks](/oss/python/langchain/messages#streaming-and-chunks) from all LLM calls in the agent. The `"updates"` mode will include completed messages with tool calls before they are routed to tools for execution.

<Note>
For a message to surface when streaming mode is `"updates"`, it must be represented in the Agent's [state](/oss/python/langchain/agents#memory) (as in the default state for [`create_agent`](/oss/python/langchain/agents)). If this is not the case, you can use [custom updates](#custom-updates) to capture the message.
</Note>
<Note>
...
def _process_stream(stream_mode: str, data: Any) -> None:
    if stream_mode == "messages":
        token, metadata = data
        if (
            metadata["langgraph_node"] == "model"
            and isinstance(token, AIMessage)  # help type-checking
        ):
            _process_message_chunk(token)  # [!code highlight]
    if stream_mode == "updates":
        for source, update in data.items():
            if source in ("model", "tools"):
                _process_completed_message(source, update["messages"][-1])  # [!code highlight]
...
for stream_mode, data in agent.stream(
    ...
    _process_stream(stream_mode, data)
```
```shell title="Output" expandable
...
def _process_interrupt(interrupt: Interrupt) -> None:  # [!code highlight]
...
def _process_stream(stream_mode: str, data: Any) -> list[Interrupt]:
    interrupts = []
    if stream_mode == "messages":
        token, metadata = data
        if (
            ...
            _process_message_chunk(token)
    if stream_mode == "updates":
        for source, update in data.items():
            if source in ("model", "tools"):
                _process_completed_message(source, update["messages"][-1])
            if source == "__interrupt__":  # [!code highlight]
                interrupts.extend(update)  # [!code highlight]
                _process_interrupt(update[0])  # [!code highlight]
    return interrupts
...
for stream_mode, data in agent.stream(
    ...
    interrupts.extend(_process_stream(stream_mode, data))  # [!code highlight]
```
```shell title="Output" expandable
...
for interrupt in interrupts:
    decisions.extend(_get_interrupt_decisions(interrupt))
...
for stream_mode, data in agent.stream(
    ...
    interrupts.extend(_process_stream(stream_mode, data))
```
```shell title="Output"
...
for _, stream_mode, data in agent.stream(
    ...
    if stream_mode == "messages":  # [!code highlight]
        if tags := data[1].get("tags", []):  # get tags from metadata  # [!code highlight]
