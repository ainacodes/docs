...
<CardGroup cols={2}>
  <Card title="Memory" href="/oss/javascript/langgraph/memory" icon="brain" arrow="true">
    Track context across multiple turns in a conversation.
  </Card>

  <Card title="Human-in-the-loop" href="/oss/javascript/langgraph/interrupts" icon="hand" arrow="true">
    Pause workflows for human inspection, approval, or modification.
  </Card>

  <Card title="Time travel" href="/oss/javascript/langgraph/use-time-travel" icon="clock-rotate-left" arrow="true">
    Replay prior executions and fork from historical states.
  </Card>

  <Card title="Fault tolerance" href="/oss/javascript/langgraph/durable-execution" icon="shield-check" arrow="true">
...
See the [Memory guide](/oss/javascript/langgraph/memory#checkpointers).
...
See the [Memory guide](/oss/javascript/langgraph/memory#stores).
...
* `@langchain/langgraph-checkpoint` - Base interface and in-memory implementation ([`MemorySaver`](https://langchain-ai.github.io/langgraphjs/reference/classes/checkpoint.MemorySaver.html))
* `@langchain/langgraph-checkpoint-sqlite` - SQLite-backed persistence ([`SqliteSaver`](https://langchain-ai.github.io/langgraphjs/reference/classes/checkpoint_sqlite.SqliteSaver.html))
* `@langchain/langgraph-checkpoint-postgres` - Postgres-backed persistence ([`PostgresSaver`](https://langchain-ai.github.io/langgraphjs/reference/classes/checkpoint_postgres.PostgresSaver.html))
...
Each checkpointer implements the [`BaseCheckpointSaver`](https://langchain-ai.github.io/langgraphjs/reference/classes/checkpoint.BaseCheckpointSaver.html) interface with the following methods:
...
<AccordionGroup>
  <Accordion title="Replay and time travel" icon="clock-rotate-left">
    ...
    LangGraph will replay steps **before** the checkpoint and re-execute steps **after** it. See the [time travel guide](/oss/javascript/langgraph/use-time-travel) for more details.
  ...
  <Accordion title="Update state" icon="pen-to-square">
    ...
    **`values`**: State updates to apply. These are passed through [reducers](/oss/javascript/langgraph/graph-api#reducers) if defined:
    ...
    **`asNode`**: Apply update as if it came from a specific node (controls which node executes next)
    ...
    See the [time travel guide](/oss/javascript/langgraph/use-time-travel) for more details on forking state.
  ...
  <Accordion title="Durable execution" icon="arrows-rotate">
    ...
    <Tip>
    To ensure workflows are deterministic and can be consistently replayed, wrap side effects and non-deterministic operations inside [tasks](/oss/javascript/langgraph/functional-api#task).
    </Tip>

    See the [durable execution guide](/oss/javascript/langgraph/durable-execution) for details.
  </Accordion>
  ...
  <Accordion title="Checkpoint data" icon="lock">
    ...
    For details on serialization options (including pickle fallback) and encryption configuration, see the [Checkpoint data section](/oss/javascript/langgraph/add-memory#checkpoint-data) in the Memory guide.
  </Accordion>
</AccordionGroup>
