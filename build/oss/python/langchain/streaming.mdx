...
1. If those messages are tracked in the [state](/oss/python/langchain/agents#memory) (as in the model node of [`create_agent`](/oss/python/langchain/agents)), use `stream_mode=["messages", "updates"]` to access completed messages through [state updates](#agent-progress).
2. If those messages are not tracked in the state, use [custom updates](#custom-updates) or aggregate the chunks during the streaming loop.

Below, we demonstrate (1). We also provide an [end-to-end example of (2)](#streaming-completed-messages-that-are-not-in-agent-state) below.
...
<Accordion title="Streaming completed messages that are not in agent state">
If you have access to the agent internals, you can use [custom updates](#custom-updates) to stream completed messages that are not reflected in [state updates](#agent-progress). Otherwise, you can aggregate message chunks during the stream (see below).
