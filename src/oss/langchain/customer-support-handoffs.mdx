...
def record_warranty_status(
    ...
    return Command(
        update={
            ...
            "current_step": "issue_classifier",  # Transition to next step
        }
    )
...
def record_issue_type(
    ...
    return Command(
        update={
            ...
            "current_step": "resolution_specialist",  # Transition to next step
        }
    )
...
Now define prompts and tools for each step. We'll use a simple dictionary to map step names to their configurations:
...
agent = create_agent(
    ...
    middleware=[apply_step_config],  # Single middleware handles all steps
...
As the agent progresses through different steps, the message history grows to include conversations from all previous steps. Without proper management, this can lead to:
- Exceeding the LLM's context window
- Slower responses and higher costs
- Agents getting confused by irrelevant messages from earlier steps

### Recommended: Use summarization

The best approach for most handoff workflows is to use [summarization](/oss/langchain/short-term-memory#summarize-messages) to compress earlier messages while preserving conversational context. This ensures later steps can reference specific details from earlier conversations without hitting context limits.

See the [short-term memory management guide](/oss/langchain/short-term-memory) for built-in summarization middleware and other techniques.

### Simpler alternative: Store facts in state

For simpler workflows where you don't need full conversational context, you can clear most message history after each transition and store only the essential facts in state. This works well when:
- Each step needs limited context from previous steps
- The workflow collects structured information (like forms or debugging flows)
- You want to minimize costs and latency

The key is to **separate what the agent sees (messages) from what it knows (state)**:

- **Message history**: Recent conversation turns for the current step
- **State**: Structured facts that carry across steps
- **System prompt**: Inject state as context so the agent knows what was learned previously

#### Extend state to store facts

For this support workflow, extend the state to track both task information and optional user facts:
...
class SupportState(AgentState):
    ...
    warranty_status: NotRequired[Literal["in_warranty", "out_of_warranty"]]
    issue_type: NotRequired[Literal["hardware", "software"]]
    # Optional: store user facts that persist across steps
    customer_name: NotRequired[str]
```

Create tools that let the agent record user facts:
...
def record_customer_info(
    ...
    """Record customer name to personalize the conversation."""
    return Command(
        update={
            "messages": [
                ToolMessage(
                    content=f"Recorded customer name: {name}",
                    tool_call_id=runtime.tool_call_id,
                )
            ],
            "customer_name": name,
...
#### Clear messages after transitions

Use `@before_model` middleware to keep only recent messages:
...
def scope_messages_to_agent(state: SupportState, runtime: Runtime) -> dict | None:
    """Keep only recent messages for the current step."""
...
#### Inject state into system prompts

Since messages are trimmed, inject the structured state into the system prompt:
...
async def apply_step_config(
    ...
    if request.state.get("warranty_status"):
        context_parts.append(f"Warranty: {request.state['warranty_status']}")
    if request.state.get("issue_type"):
        context_parts.append(f"Issue: {request.state['issue_type']}")
    if request.state.get("customer_name"):
        context_parts.append(f"Customer: {request.state['customer_name']}")

    context_summary = ""
    if context_parts:
        context_summary = "\n\nContext from previous steps: " + ", ".join(context_parts)

    # Format prompt and inject context
    system_prompt = stage_config["prompt"].format(**request.state)
