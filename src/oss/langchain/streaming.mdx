...
<Note>
For a message to surface when streaming mode is `"updates"`, it must be represented in the Agent's [state](/oss/langchain/agents#memory) (as in the default state for [`create_agent`](/oss/langchain/agents)). If this is not the case, you can use [custom updates](#custom-updates) to capture the message.
</Note>
<Note>
